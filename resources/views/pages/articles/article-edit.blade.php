    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-6 col-md-12 col-sm-12">
                 <form id="contact-forms" class="00" method="post" action="{{route('articles.update',['article'=>$article->id])}}">
                @csrf
                @method('PUT')
                 <!-- form message -->
                    <div class="row">
                    ...
                    <!-- end message -->
                    <span class="text-color">Edit Article</span>
                    <h3 class="text-md mb-4">New Article post</h3>
                    <div class="form-group">
                        <input name="title" value="{{$article->title}}" type="text" class="form-control" placeholder="Title">
                        @error('title')
                    ...
                    <div class="form-group-2 mb-4">
                        <textarea name="description" class="form-control" rows="7" placeholder="Description">{{$article->description}}</textarea>
                        @error('description')
                            <div class="alert alert-danger mt-3">{{ $message }}</div>
                        @enderror
                    </div>
